 ...
 */
class W3Site_Forum_Model_Forum_Url extends Mage_Core_Model_Config_Data
{
    protected function _beforeSave()
    {
        $rewrite = Mage::getModel("core/url_rewrite")->loadByIdPath('forum');
        $value = $this->getValue();
        
        if (!$value && $rewrite->getUrlRewriteId()){
            $rewrite->remove();
            return;
        }
        elseIf(!$value){
